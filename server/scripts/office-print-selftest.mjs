...
const p240 = readFileSync(join(PATCH_DIR, '0240-hooks-webui-register-print.patch'), 'utf8');
check('0240 hook：WebUIConfig 注册挂接', /RegisterNodeByteWebUIConfigs/.test(p240));
check('0240 hook：Print() 重定向（策略门控）', /Print\(Browser\* browser\)/.test(p240) && /PrintPanelEnabled/.test(p240));
