...
全部服务嵌在**一个镜像**内（PostgreSQL + RustFS 对象存储 + WS 信令 + Web + Nginx 入口），一条命令直接部署：
...
| 事项 | 说明 |
|---|---|
| 数据卷 | `/data`（PG 数据 + RustFS 对象 + 自动密钥），备份即 `tar` 该卷 |
...
架构：Nginx(:8080) → Next.js(:3000) / ws-service(:8081) / RustFS(:9000，仅桶名反代)；PostgreSQL(:5432) 由业务服务内网直连；六进程由 supervisord 托管、日志透出 `docker logs`。
...
包含：PostgreSQL（init.sql 自动执行）+ RustFS（七桶自动初始化，`s3-init` 容器）+ WS 信令服务 + Next.js standalone。
...
## 4. S3 桶规划（RustFS / MinIO 兼容）（提示词 5.11.3）
