 ...
 * 等 MinIO 就绪（最多 ~120s）→ 逐个建桶（已存在则跳过）→ 退出码 0
 */
...
for (let i = 1; i <= 40; i++) {
  try {
    await ready(mc);
    console.log(`[bucket-init] MinIO ready at ${ENDPOINT}:${PORT}`);
    break;
  } catch (e) {
    if (i === 40) {
      console.error('[bucket-init] MinIO not reachable, giving up:', e.message);
      process.exit(1);
    }
    await sleep(3000);
  }
